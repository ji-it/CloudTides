...
def setup_args():
    ...
    parser.add_argument('--vm-name',
                        required=True,
                        help='Name of the VM deployed by OVA')
...
def main():
    ...
    network = get_network(dc, args.network)
    ...
    cisp = vim.OvfManager.CreateImportSpecParams(diskProvisioning="thin", 
        entityName=args.vm_name, networkMapping=nma)
    ...
    tasks.wait_for_tasks(si, [power_task])
    print("Done.")
...
def get_network(dc, name):
    """
    Pick a network by its name.
    """
    for nw in dc.network:
        try:
            if nw.name == name:
                return nw
        except:  # Ignore datastores that have issues
